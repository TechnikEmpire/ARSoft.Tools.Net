...
namespace ARSoft.Tools.Net.Dns
{
    ...
    public abstract class DnsMessageBase
    {
        ...
        public List<DnsRecordBase> AdditionalRecords
        {
            ...
            set
            {
                _additionalRecords = (value ?? new List<DnsRecordBase>());
            }
        }

        internal abstract bool IsTcpUsingRequested
        {
            get;
        }

        internal abstract bool IsTcpResendingRequested
        {
            get;
        }
        ...
        public OperationCode OperationCode
        {
            get
            {
                return (OperationCode)((Flags & 0x7800) >> 11);
            }
            set
            {
        ...
        private ReturnCode ValidateTSig(byte[] resultData, DnsServer.SelectTsigKey tsigKeySelector, byte[] originalMac)
        {
            ...
            else
            {
                ...
                // maxLength for the buffer to validate: Original (unsigned) dns message and encoded
                // TSigOptions because of compression of keyname, the size of the signed message can
                // not be used
                int maxLength = TSigOptions.StartPosition + TSigOptions.MaximumLength;
                if (originalMac != null)
                {
                    // add length of mac on responses. MacSize not neccessary, this field is allready
                    // included in the size of the tsig options
        ...
        #endregion Methods for parsing answer
        ...
        #endregion Helper methods for parsing records
