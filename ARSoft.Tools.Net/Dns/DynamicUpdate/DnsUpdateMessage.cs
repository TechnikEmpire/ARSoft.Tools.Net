...
namespace ARSoft.Tools.Net.Dns.DynamicUpdate
{
    ...
    public class DnsUpdateMessage : DnsMessageBase
    {
        ...
        public DomainName ZoneName
        {
            get
            {
                return Questions.Count > 0 ? Questions[0].Name : null;
            }
            set
            {
                Questions = new List<DnsQuestion>() { new DnsQuestion(value, RecordType.Soa, RecordClass.INet) };
            }
        }
        ...
        public List<PrequisiteBase> Prequisites
        {
            get
            {
                return _prequisites ?? (_prequisites = new List<PrequisiteBase>());
            }
            set
            {
                _prequisites = value;
        ...
        public List<UpdateBase> Updates
        {
            get
            {
                return _updates ?? (_updates = new List<UpdateBase>());
            }
            set
            {
                _updates = value;
